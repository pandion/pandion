...
function mousemenu ( jid, res, group )
{
	...
	var Online = external.globals( 'ClientRoster' ).Items.Exists( jid ) && external.globals( 'ClientRoster' ).Items.Item( jid ).Resources.Count;
...
//	menu.AddItem( Online,	false, false, false, 0, external.globals( 'Translator' ).Translate( 'main', 'menu_filetransfer' ),	10 );
	...
	for ( var i = 0; i < keys.length; i++ )
		if ( ! external.globals( 'ClientRoster' ).Groups.Item( keys[i] ).Items.Exists( jid ) )
			groups.push( keys[i] );
	groups = groups.sort();
	...
	switch ( menu.Choice )
	{
		...
		case   6: external.globals( 'ClientRoster' ).Items.Item( jid ).ChangeName(); break;
		case   7: dial_block( jid ); break;
		case   8: external.globals( 'ClientRoster' ).Items.Item( jid ).Purge( false, group ); break;
		case   9: external.globals( 'ClientRoster' ).Items.Item( jid ).ReRequest(); break;
		...
		default:
			if ( menu.Choice > 100 && menu.Choice - 101 < rooms.length )
			{
				if ( external.globals( 'ConferenceSessionPool' ).Trackers.Exists( rooms[ menu.Choice - 101 ] ) )
					dial_conference_invite( external.globals( 'ConferenceSessionPool' ).Trackers.Item( rooms[ menu.Choice - 101 ] ), jid, '' );
			}
			else if ( menu.Choice > 200 && menu.Choice - 201 < groups.length )
				external.globals( 'ClientRoster' ).Items.Item( jid ).CC( group, groups[ menu.Choice - 201 ] );
			else if ( menu.Choice > 300 && menu.Choice - 301 < groups.length )
				external.globals( 'ClientRoster' ).Items.Item( jid ).CC( '', groups[ menu.Choice - 301 ] );
