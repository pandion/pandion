...
HTTPClient::HTTPClient(HINTERNET hInternet, HTTPEventRouter* eventRouter,
	DWORD sessionID) :
	...
	USHORT port)
{
	...
	DWORD threadID;
	::CreateThread(NULL, 0, UploadFileProc, (LPVOID) this, 0, &threadID);
}
DWORD HTTPClient::GetProgress()
{
	return m_Progress;
}
DWORD HTTPClient::GetLength()
{
	return m_Length;
}
...
DWORD __stdcall HTTPClient::DownloadFileProc(void *pThis)
{
	return ((HTTPClient*) pThis)->DownloadFile();
}
DWORD __stdcall HTTPClient::UploadFileProc(void *pThis)
{
	return ((HTTPClient*) pThis)->UploadFile();
}
...
DWORD HTTPClient::UploadFile()
{
	HANDLE hLocalFile = ::CreateFile(m_FileName.c_str(), GENERIC_READ,
	...
	if(hLocalFile)
	{
		...
		if(hRemoteFile)
		{
			HINTERNET hRequest = ::HttpOpenRequest(hRemoteFile, L"POST",
				m_URI.c_str(), NULL, NULL, NULL, INTERNET_FLAG_NO_CACHE_WRITE,
				NULL);
			if(hRequest)
			{
				DWORD fileSizeHigh, bytesRead = 0;
				DWORD fileSize = ::GetFileSize(hLocalFile, &fileSizeHigh);
				m_Length = fileSize;
				...
				internetBuffers.dwHeadersTotal = 0;
				internetBuffers.lpvBuffer = NULL;
				internetBuffers.dwBufferLength = 0;
				internetBuffers.dwBufferTotal = fileSize;
				internetBuffers.dwOffsetLow = 0;
				internetBuffers.dwOffsetHigh = 0;

				BOOL success = ::HttpSendRequestEx(hRequest, &internetBuffers,
					NULL, 0, m_SessionID);
				...
				if(success)
				{
					...
					while(!m_Abort)
					{
						success = ::ReadFile(hLocalFile, buffer,
							4096, &bytesRead, NULL);
						if(!success)
						{
							m_EventRouter->OnFileErr(m_SessionID,
								::GetLastError());
							m_Abort = true;
							break;
						}
						...
						else
						{
							...
							{
								success = ::InternetWriteFile(hRequest,
